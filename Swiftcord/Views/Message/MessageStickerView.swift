...
struct StickerPackView: View {
	...
	@State private var stickerHovered: Int? = nil
					...
					ForEach(0..<Int(ceil(Double(pack.stickers.count)/3.0)), id: \.self) { row in
						HStack {
							ForEach(0..<min(3, Int(pack.stickers.count - row * 3)), id: \.self) { column in
								let n: Int = row*3+column
								StickerItemView(sticker: pack.stickers[n], size: 95, play: .onHover)
									.onHover {
										stickerHovered = $0 ? n : nil
									}
									.scaleEffect((stickerHovered == n) ? 1.1 : 1.0)
									.opacity((stickerHovered == n) ? 1 : 0.5)
...
struct MessageStickerView: View {
	...
	private func loadStickerPack() async -> StickerPack? {
		...
		for pack in stickerPacks {
			if pack.id == fullSticker!.pack_id {
				return pack
			}
		}
		return nil
